...
as 
  ...
  is 
    ...
    l_static_id        flow_object_attributes.obat_vc_value%type;
    l_subject          flow_object_attributes.obat_vc_value%type;
    l_parameters       flow_object_attributes.obat_clob_value%type;
    l_business_ref     flow_object_attributes.obat_vc_value%type;
    l_task_comment     flow_object_attributes.obat_vc_value%type;
    ...
    if flow_apex_env.ver_le_21_2 then 
      ...
      for rec in (
          ...
          and obat.obat_key in ( flow_constants_pkg.gc_apex_usertask_application_id,
                                 flow_constants_pkg.gc_apex_usertask_static_id,            
                                 flow_constants_pkg.gc_apex_usertask_subject,            
                                 flow_constants_pkg.gc_apex_usertask_parameters,     
                                 flow_constants_pkg.gc_apex_usertask_business_ref,      
                                 flow_constants_pkg.gc_apex_usertask_task_comment
                              )
      )
      loop
        case rec.obat_key
          ...
          when flow_constants_pkg.gc_apex_usertask_static_id then
            l_static_id := rec.obat_vc_value;
            flow_proc_vars_int.do_substitution( pi_prcs_id => l_prcs_id, pi_sbfl_id => l_sbfl_id, pi_scope => l_scope, pio_string => l_static_id);
          when flow_constants_pkg.gc_apex_usertask_subject then
          ...
          when flow_constants_pkg.gc_apex_usertask_parameters then
            l_parameters := rec.obat_clob_value;
            flow_proc_vars_int.do_substitution( pi_prcs_id => l_prcs_id, pi_sbfl_id => l_sbfl_id, pi_scope => l_scope, pio_string => l_parameters);
          when flow_constants_pkg.gc_apex_usertask_business_ref then
            l_business_ref := rec.obat_vc_value;
            flow_proc_vars_int.do_substitution( pi_prcs_id => l_prcs_id, pi_sbfl_id => l_sbfl_id, pi_scope => l_scope, pio_string => l_business_ref);
          when flow_constants_pkg.gc_apex_usertask_task_comment then
            l_task_comment:= rec.obat_vc_value;
  ...
  is 
    l_usertask_type    flow_object_attributes.obat_vc_value%type;
  begin
    ...
    l_usertask_type := flow_engine_util.get_object_vc_attribute( p_objt_bpmn_id  => p_step_info.target_objt_ref
                                                               , p_dgrm_id  => p_sbfl_info.sbfl_dgrm_id
                                                               , p_attribute_key  => flow_constants_pkg.gc_task_type_key
                                                               );
